...
class BlinkMorseProcessor(VideoProcessorBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        with self.lock:
            ...
            # -- on-frame overlay (kept minimal; full status lives in the sidebar panel) --
...
# ----------------------------------------------------------------------
# 6. LIVE POLLING LOOP
#    Pushes the processor's settings each run and keeps the status
#    panel refreshed while the stream is active.
# ----------------------------------------------------------------------
if ctx.video_processor:
    ...
    while ctx.state.playing:
        ...
        ear_placeholder.metric("EAR", f"{state['ear']:.3f}")
        status_placeholder.metric("Eye Status", state["status"])
        code_placeholder.text(f"Current Morse Code: {state['morse'] or '—'}")
        message_placeholder.text(f"Decoded Message: {state['message'] or '—'}")
